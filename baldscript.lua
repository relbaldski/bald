...
TrigonVer = "Trigon v0.04a"
print(TrigonVer)

if game.CoreGui:FindFirstChild("TrigonMain") then
    game.CoreGui["TrigonMain"]:Destroy()
end
...
local b = game:GetService('Players').LocalPlayer.Name ..'TrigonConfigs.json'
...
trigok.TrigonMain.Name = "TrigonMain"
...
task.spawn(function()
	...
	local function initializeCheckBoxes()
		for value, button in pairs(checkboxList) do
			if Settings[value] then
				print(value, button, Settings[value])
				cboxCheck(button) 
			else
				cboxUnCheck(button) 
			end
...
task.spawn(function()
	...
	lsf = gethui().TrigonMain.MainFrame.homeFrame.localscriptsFrame
	alsf = gethui().TrigonMain.MainFrame.homeFrame.addlocalscriptsFrame
